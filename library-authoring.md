...
The standard library (`facet/std`) is always available in root scope without an explicit import — `Cube`, `Move`, `Sin`, etc. all work out of the box. Other built-in libraries like `facet/threads` require an explicit `lib` import.
...
fn Fastener.HexNut() Solid {
    ...
    var hex = Polygon(for i [0:<6] {
        yield Vec2(Cos(i * 60 deg) * cr, Sin(i * 60 deg) * cr);
    }).Extrude(self.nut_h);
...
fn Fastener.HexBolt(Length length) Solid {
    ...
    var shaft = T.Thread(self.size).Outside(length)
        .Move(0 mm, 0 mm, self.bolt_head_h);
    return head + shaft;
}
```
